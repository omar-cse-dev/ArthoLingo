#!/usr/bin/env python3
"""Build/validate an ArthoLingo local English→Bengali allowlist dictionary.

Input: a CSV with columns word,meaning_bn (extra columns are allowed).
Output: validated JSON suitable for ./data/artholingo-dictionary.json
...
def main() -> None:
    ...
    ap.add_argument('csv')
    ap.add_argument('-o','--output', default='data/artholingo-dictionary.json')
    args = ap.parse_args()
    entries = {}
    rejected = 0
    with open(args.csv, newline='', encoding='utf-8-sig') as f:
        ...
        for row in reader:
            ...
            entries[word] = {
                ...
                'approved': True,
                'safe': True,
                'source': row.get('source') or 'Imported + ArthoLingo filtered'
            }
    if not entries:
