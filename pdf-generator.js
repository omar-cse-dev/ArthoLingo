/* ============================================================
   ArthoLingo - PDF Export Engine v2.6
...
window.ArthoLingoPDF = (function () {
    ...
    return `
      * { box-sizing: border-box; }
      ...
      .page {
        ...
        position: relative;
        overflow: hidden;
        padding: ${MARGIN.top}px ${MARGIN.right}px ${MARGIN.bottom + FOOTER_H + 18}px ${MARGIN.left}px;
        font-synthesis: none;
  ...
  function makePage(config, title, subtitle, bodyHtml, pageNumber) {
    ...
    page.innerHTML = `
      ...
      <div class="footer-zone">
        <div class="footer-name">Omar Mohammad Chowdhury</div>
        <div class="footer-meta">ArthoLingo Learning PDF</div>
  ...
  async function ensureScript(src, globalName) {
    ...
    await new Promise((resolve, reject) => {
      const existing = document.querySelector(`script[data-artholingo-lib="${globalName}"]`);
      if (existing) {
      ...
      script.src = src;
      script.async = false;
      script.dataset.artholingoLib = globalName;
  ...
  function showDownloadReady(pdf, filename) {
    const old = document.getElementById('artholingo-pdf-download-ready');
    if (old) old.remove();
    ...
    overlay.id = 'artholingo-pdf-download-ready';
  ...
  async function buildPages(config) {
    ...
    const pages = [];
    const maxItemsPerPage = config.mode === 'vocab' ? 18 : 7;
    const source = config.mode === 'vocab' ? config.vocab : config.sentences;

    if (!source?.length) throw new Error('No content available for PDF export.');

    for (let i = 0; i < source.length; i += maxItemsPerPage) {
      const chunk = source.slice(i, i + maxItemsPerPage);
      let bodyHtml = '';
      if (config.mode === 'vocab') {
        bodyHtml += `<div class="table-head"><div class="vocab-head">#</div><div class="vocab-head">English Word</div><div class="vocab-head">Bengali Meaning</div></div>`;
        bodyHtml += chunk.map((item, idx) => `
          <div class="vocab-row">
            <div class="vocab-num">${i + idx + 1}</div>
            <div class="vocab-word">${esc(item.word)}</div>
            <div class="vocab-meaning">${esc(item.meaning)}</div>
          </div>
        `).join('');
      } else {
        bodyHtml = chunk.map(item => `
      ...
      const page = makePage(
        config,
        config.mode === 'vocab' ? 'ArthoLingo Vocabulary' : 'ArthoLingo Sentences',
        config.mode === 'vocab' ? 'English words and approved Bengali meanings' : 'English sentences and Bengali translations',
        bodyHtml,
        pages.length + 1
      );
    ...
    for (const page of pages) {
      ...
      if (content.scrollHeight <= CONTENT_HEIGHT + 4) {
        corrected.push(page);
        continue;
      }

      const children = Array.from(content.children).filter(el => !el.classList.contains('page-title') && !el.classList.contains('subtitle'));
      page.remove();
      const title = config.mode === 'vocab' ? 'ArthoLingo Vocabulary' : 'ArthoLingo Sentences';
      const subtitle = config.mode === 'vocab' ? 'English words and approved Bengali meanings' : 'English sentences and Bengali translations';
      let chunkHtml = '';

      for (const child of children) {
        const candidate = makePage(config, title, subtitle, chunkHtml + child.outerHTML, corrected.length + 1);
        wrapper.appendChild(candidate);
        const h = candidate.querySelector('.content').scrollHeight;
        candidate.remove();

        if (h > CONTENT_HEIGHT && chunkHtml) {
          const finalPage = makePage(config, title, subtitle, chunkHtml, corrected.length + 1);
          wrapper.appendChild(finalPage);
          corrected.push(finalPage);
          chunkHtml = child.outerHTML;
        } else {
          chunkHtml += child.outerHTML;
        }
      }
    ...
    return { wrapper, pages: corrected };
  }
  ...
  async function generate(config) {
    ...
    try {
      ...
      const filename = config.mode === 'vocab'
        ? `ArthoLingo_Vocabulary_${date}.pdf`
        : `ArthoLingo_Sentences_${date}.pdf`;
