...
const CACHE_NAME = 'artholingo-v2.7.0';
const STATIC_CACHE = 'artholingo-static-v2.3.0';
const DYNAMIC_CACHE = 'artholingo-dynamic-v2.3.0';
...
const PRECACHE_ASSETS = [
  ...
  './artholingo-dictionary.json',
  './english-lexicon-10k.txt',
  './content-safety.js',
  './word-classes.js',
  './manifest.json',
  './favicon.svg',
...
self.addEventListener('install', event => {
  console.log('[SW] Installing v2.7.0...');
  event.waitUntil(
...
self.addEventListener('activate', event => {
  console.log('[SW] Activating v2.7.0...');
  event.waitUntil(
    caches.keys()
      .then(keys => Promise.all(
        keys
          .filter(key => key.startsWith('artholingo-') && key !== STATIC_CACHE && key !== DYNAMIC_CACHE)
          .map(key => {
            console.log('[SW] Deleting old ArthoLingo cache:', key);
...
self.addEventListener('push', event => {
  const data = event.data?.json() || { title: 'ArthoLingo', body: 'New update available.' };
  event.waitUntil(
...
console.log('[SW] ArthoLingo Service Worker v2.7.0 loaded.');
